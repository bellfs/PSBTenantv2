...
const PLATFORM_BASE_URL = process.env.PLATFORM_URL || 'https://maintenance.52oldelvet.com';

async function sendNewIssueEmail({ issue, tenant, property, messages, attachments }) {
  ...
  const issueUrl = `${PLATFORM_BASE_URL}/issues/${issue.id}`;

  const html = `
    <div style="font-family: 'Helvetica Neue', Arial, sans-serif; max-width: 680px; margin: 0 auto;">
      <div style="background: #0d47a1; color: white; padding: 24px 32px; border-radius: 12px 12px 0 0;">
        <h1 style="margin: 0; font-size: 20px; font-weight: 500;">🆕 New Maintenance Issue Reported</h1>
        <p style="margin: 4px 0 0 0; opacity: 0.7; font-size: 14px;">Ref: ${issue.uuid} | Reported via WhatsApp</p>
      </div>
      ...
      <div style="background: #f8f9fa; padding: 24px 32px; border: 1px solid #e0e0e0;">
        <table style="width: 100%; font-size: 14px; border-collapse: collapse;">
          ...
          <tr>
            ...
            <td style="padding: 8px 0;">${issue.category || 'Pending AI diagnosis'}</td>
          </tr>
          <tr>
          ...
          <tr>
            ...
            <td style="padding: 8px 0;">${issue.ai_diagnosis || 'Analysis in progress...'}</td>
          </tr>
          <tr>
      ...
      ${emailAttachments.length > 0 ? `
        <div style="padding: 24px 32px; border: 1px solid #e0e0e0; border-top: none;">
          <h3 style="margin: 0 0 8px 0; font-size: 16px;">📎 ${emailAttachments.length} Photo(s) Attached</h3>
          <p style="color: #666; font-size: 13px; margin: 0;">Photos from the tenant are attached to this email.</p>
        </div>
      ` : ''}

      <div style="padding: 24px 32px; border: 1px solid #e0e0e0; border-top: none; border-radius: 0 0 12px 12px; text-align: center;">
        <a href="${issueUrl}" style="display: inline-block; background: #0d47a1; color: white; padding: 12px 32px; border-radius: 8px; text-decoration: none; font-weight: 600; font-size: 14px;">View Issue on Platform →</a>
      </div>

      <div style="padding: 16px 32px; text-align: center; color: #999; font-size: 12px;">
        PSB Properties Maintenance Hub | New Issue Notification
      </div>
    </div>`;
  ...
  const mailOptions = {
    ...
    subject: `[NEW] Maintenance Issue ${issue.uuid} - ${issue.title || 'New Issue'} - ${property?.name || 'Property'}`,
    html: html,
  ...
  try {
    ...
    console.log('[Email] New issue email sent for issue', issue.uuid);
  } catch (err) {
    console.error('[Email] Failed to send new issue email:', err.message);
  }
}
    ...
    <div style="font-family: 'Helvetica Neue', Arial, sans-serif; max-width: 680px; margin: 0 auto;">
      ...
      <div style="background: #f8f9fa; padding: 24px 32px; border: 1px solid #e0e0e0;">
        <table style="width: 100%; font-size: 14px; border-collapse: collapse;">
          ...
          ${issue.estimated_cost ? `<tr>
            <td style="padding: 8px 0; color: #666;">Est. Cost</td>
            <td style="padding: 8px 0;">£${issue.estimated_cost}</td>
          </tr>` : ''}
          ${issue.estimated_hours ? `<tr>
            <td style="padding: 8px 0; color: #666;">Est. Hours</td>
            <td style="padding: 8px 0;">${issue.estimated_hours}h</td>
          </tr>` : ''}
          <tr>
            <td style="padding: 8px 0; color: #666;">Reported</td>
            <td style="padding: 8px 0;">${new Date(issue.created_at).toLocaleString('en-GB')}</td>
      ...
      <div style="padding: 24px 32px; border: 1px solid #e0e0e0; border-top: none; border-radius: 0 0 12px 12px; text-align: center;">
        <a href="${issueUrl}" style="display: inline-block; background: #B71C1C; color: white; padding: 12px 32px; border-radius: 8px; text-decoration: none; font-weight: 600; font-size: 14px;">View Issue on Platform →</a>
      </div>
